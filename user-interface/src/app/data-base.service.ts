...
export class DataBaseService {
  ...
  getPlayerList(): Observable<any> {
    return this.http.post(`${this.baseUrl}` + `/players/filter?start=0&end=19`,
      {
        operatorType: 'NO_FILTER',
        value: ''
      });
  }
  getNextPlayer(start, end): Observable<any> {
    return this.http.post(`${this.baseUrl}` + `/players/filter?start=${start}&end=${end}`, {
      operatorType: 'NO_FILTER',
      value: ''
    })
  }
